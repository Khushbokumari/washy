...
class CategoryTabItem extends StatefulWidget {
  ...
  const CategoryTabItem(this.serviceId, this.categoryName, this.product,
      {Key key})
      : super(key: key);

  @override
  CategoryTabItemState createState() => CategoryTabItemState();
}

class CategoryTabItemState extends State<CategoryTabItem> {
  bool expanded = false;
  ...
  Widget _getRemoveButton(
    ...
    return IconButton(
      ...
      onPressed: () => _productRemoveCallback(serviceId),
    );
  }
  ...
  Widget _getPortraitLayout(
    ...
    return Column(
      children: [
        SizedBox(
          height: CategoryTabItem.MAX_HEIGHT_PORTRAIT,
        ...
        Row(
          ...
          children: [
            Container(
              ...
              height: CategoryTabItem.MAX_HEIGHT_LANDSCAPE,
  ...
  Future<void> _productRemoveCallback(String serviceId) async {
    ...
    if (!isDone) {
      bool val = await showGotoCheckoutPopup();
      if (val) {
  ...
  Future<bool> showGotoCheckoutPopup() async {
